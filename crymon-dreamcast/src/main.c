 ...
 *
 * Player sprite: sprites_mason.h is generated by tools/gen_sprites.py
 * from xenodorian/CryMon's public/sprites/mason/{down,up,left,right}
 * -1.png -- the reference game's real pixel art for its protagonist,
 * not placeholder art. Props (bed/shelf/crate) still draw as
 * flat-colored rectangles; only the player sprite is wired up in this
 * step, to verify sprite loading/blitting works at all before
 * spending more time on a full asset pipeline.
 */
...
/* Blits a MASON_SPRITE_W x MASON_SPRITE_H RGB565 sprite (see
   sprites_mason.h) with its top-left corner at (x, y), skipping any
   pixel equal to MASON_SPRITE_KEY (color-keyed transparency -- the
   sprite has no separate alpha channel once baked into this array). */
static void blit_sprite(const u16 *px, int x, int y) {
    int sx, sy;
    for(sy = 0; sy < MASON_SPRITE_H; sy++) {
        for(sx = 0; sx < MASON_SPRITE_W; sx++) {
            u16 c = px[sy * MASON_SPRITE_W + sx];
            if(c != MASON_SPRITE_KEY)
...
static void draw_player(int cx, int cy, int dir) {
    static const u16 *const frames[4] = {
        mason_down, mason_up, mason_left, mason_right
    };
    int x = cx - MASON_SPRITE_W / 2, y = cy - MASON_SPRITE_H;
    blit_sprite(frames[dir], x, y);
}
